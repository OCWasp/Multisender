...
contract MultiSender is IAcceptTokensTransferCallback {
    ...
    address static _senderAddr;
    address static _tokenRootAddr;

    constructor() public {
        require(msg.sender == _creator, 1001);
        require(_senderAddr.value != 0, 1001);
        require(_tokenRootAddr.value != 0, 1001);
        tvm.accept();
    }
    ...
    */
    ...
    function getTransferNumber() external view returns (uint128) {
    ...
    */
    ...
    function getTransferGas() external view returns (uint128) {
        return transferGas;
    }
