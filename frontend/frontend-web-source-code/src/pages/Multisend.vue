      ...
      >
          ...
          <div class="row col justify-center">
            <div class="col-11 row inTitle q-my-sm" style="display: flex;align-items:center">
              ...
              <span v-if="selToken && isLogin" style="display: flex;align-items:center">
                balance:
                <span class="q-ml-sm" v-if="!loading.withdrawalBalance">{{ withdrawalBalance }} {{ selToken.label }}</span>
                <q-skeleton v-if="loading.withdrawalBalance" class="text-h5 q-ml-sm" width="50px" type="text" />
                <q-btn v-if="!loading.withdrawalBalance && withdrawalBalance > 0" class="but-bg1 fontStyle1 text-capitalize q-ml-sm" dense label="withdraw" :disable="loading.withdraw" :loading="loading.withdraw" v-check-auth-on:click="() => withdraw()"/>
              </span>
            </div>
        ...
        <template v-slot:navigation>
          <q-stepper-navigation>
            ...
            <q-btn v-if="step !== 3" :disable="loading.withdraw || loading.summaryloading || (step === 2 && summaryData.totalTokens === 0) || (step === 2 && summaryData.Balance < summaryData.ReqApproveAmount)" :loading="loading.toContractloading"   v-check-auth-on:click="() => next()" :label="step === 3 ? 'Send' : 'Next'" class="but-bg1 fontStyle1 text-capitalize q-ml-sm" />
          </q-stepper-navigation>
        </template>
      </q-stepper>
    </div>
...
export default ({
  ...
  methods: {
    ...
    checkBatchTransfer (tempBalance) {
      console.log('checkBatchTransfer ...')
      this.checkCount++
      EverWallet.getDeposit(this.selToken.decimals).then(contractBalance => {
        this.this.withdrawalBalance = contractBalance
