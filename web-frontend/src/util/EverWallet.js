...
const contractAddress = '0:1233c5917bb0449cab365ef9d76e97c0a1819cec577bbed9f54e80b8b39e278b'
...
export default {
  ...
  async getDepositBySender (sender, tokenRoot, decimals) {
    try {
      let rootAcc = new Account('', { address: tokenRoot, client })
      rootAcc.abi = { type: 'Contract', value: rootAbi }
      let response = await rootAcc.runLocal('walletOf', { answerId: 1, walletOwner: contractAddress })
      const contractWalletAddress = response.decoded.output.value0
      rootAcc = new Account('', { address: contractAddress, client })
      rootAcc.abi = { type: 'Contract', value: walletAbi }
      response = await rootAcc.runLocal('getDepositBySender', { sender: sender, walletAddress: contractWalletAddress })
