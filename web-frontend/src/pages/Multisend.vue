<template>
  <q-page class="q-mb-xl">
    ...
    <div class="row col justify-center q-mt-lg" style="min-width:450px;">
      <q-stepper
        v-model="step"
        ref="stepper"
        animated
        ...
        class="multisendCard col-md-8 col-sm-10 col-xs-12"
      >
        <!-- 步骤 1 -->
        <q-step :name="1" title="Prepare" prefix="1" :done="step > 1">
          <div class="row col justify-center">
            <div class="col-11 inTitle q-my-sm">Token</div>
            ...
            <div  v-if="showUpLoadFile"  class="col-11 multisendCard" style="height:200px">
              <div  class="full-width row justify-center q-mt-xl" style="height:100px">
                ...
                <q-icon v-if="!loading.uploading" class="pointer" size="xl" name="img:/icons/upload.png" @click="importFile"/>
                <q-spinner v-if="loading.uploading" class="q-mr-sm" color="white" size="3.5em"/>
        ...
        <q-step :name="2" title="Confirm" prefix="2" :done="step > 2">
          <div class="row col justify-center">
            ...
            <div v-if="!loading.summaryloading" class="multisendCard contentFont row col col-11 justify-center">
              <q-item class="col-xs-12 col-sm-6 borderStyle1">
                <q-item-section>
            ...
            <div v-if="loading.summaryloading" class="multisendCard contentFont row col col-11 justify-center">
              <q-item v-for="(i, index) in 6" :key="index" class="col-xs-12 col-sm-6 borderStyle1">
                <q-item-section>
            ...
            <div v-if="hasToContract" class="col-11 alertFont q-my-sm text-secondary">Request approve succceeded! Please click next</div>
             <div v-if="loading.toContractloading" class="col-11 alertFont q-my-sm text-info">Please wait a while after clicking the send button,Wait for the {{hasToContract ? 'transfer ' : 'Request approve succceeded '}} succceeded</div>
        ...
        <q-step :name="3" title="Send" prefix="3" icon="add_comment">
          <div class="row col justify-center">
            <div class="col-11 sendTitle text-center">total of {{summaryData.totalTran}} needs to be sent, {{summaryData.totalTran}} have been sent {{batchSuccess?'successfully':'fail'}}</div>
        ...
        <template v-slot:navigation>
          <q-stepper-navigation>
            <q-btn :disable="loading.toContractloading || loading.summaryloading" v-if="step > 1" flat  @click="$refs.stepper.previous(), previous()" icon="keyboard_arrow_left" class="but-bg1 fontStyle1 text-capitalize" />
            <q-btn v-if="step !== 3" :disable="loading.summaryloading || (step === 2 && summaryData.totalTokens === 0) || (step === 2 && summaryData.Balance < summaryData.ReqApproveAmount)" :loading="loading.toContractloading" @click="$refs.stepper.next(), next()"  :label="step === 3 ? 'Send' : 'Next'" class="but-bg1 fontStyle1 text-capitalize q-ml-sm" />
          </q-stepper-navigation>
        </template>
      </q-stepper>
    </div>
  ...
  methods: {
    ...
    async transfer () {
      ...
      if (!this.hasToContract) {
        ...
        const tempFlag = await EverWallet.batchTransfer(this.selToken.address, this.user.address, this.confirmData, this.selToken.decimals, this.summaryData.gas)
        if (tempFlag) {
          this.renewBalance(await EverWallet.getBalance(this.user.address))
          const tempBalance = this.summaryData.currentAllowance - this.summaryData.totalTokens
          this.batchSuccess = false
          this.stopInterval = false
          this.timer = setInterval(() => {
            this.checkCompleted(tempBalance)
    ...
    checkCompleted (tempBalance) {
      console.log('checkCompleted ...')
      this.checkCount++
      // 获取最余额对比
      EverWallet.getDepositBySender(this.user.address, this.selToken.address, this.selToken.decimals).then(contractBalance => {
    ...
    async loadSummaryData () {
      ...
      this.summaryData.currentAllowance = await EverWallet.getDepositBySender(this.user.address, this.selToken.address, this.selToken.decimals)
      EverWallet.getDeposits()
    ...
    async renewSummaryData () {
      ...
      this.summaryData.temptotalTokens = this.confirmData.length === 0 ? 0 : await EverWallet.getTotalAmount(this.confirmData, this.selToken.decimals)
      this.summaryData.totalTokens = this.summaryData.temptotalTokens / Math.pow(10, this.selToken.decimals)
      if (this.summaryData.currentAllowance >= this.summaryData.totalTokens) {
        ...
        this.summaryData.ReqApproveAmount = (this.summaryData.temptotalTokens - this.summaryData.currentAllowance * Math.pow(10, this.selToken.decimals)) / Math.pow(10, this.selToken.decimals)
      }
      this.loading.summaryloading = false
    },
    /**
